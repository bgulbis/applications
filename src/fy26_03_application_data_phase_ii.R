...
raw_apps <- read_csv(paste0(f, "raw/pgy1_application_scores_fy26_phase_ii.csv")) |> 
    filter(str_detect(designation_0, "Phase II") | str_detect(designation_1, "Phase II"))
...
data_app_scores <- raw_apps |>
    ...
    mutate(
        across(name, \(x) str_remove_all(x, "phase_ii_")),
...
openxlsx::write.xlsx(
    total_score,
    paste0(f, "final/2026_pgy1_applications_phase_ii.xlsx")
)
