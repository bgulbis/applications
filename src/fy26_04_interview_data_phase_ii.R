...
raw_apps <- read_csv(paste0(f, "raw/pgy1_interview_scores_fy26_phase_ii.csv")) |> 
...
data_application <- raw_apps |>
    # mutate(across(name, \(x) str_remove_all(x, "phase_ii_"))) |> 
...
data_interviews <- raw_apps |>
    ...
    mutate(
        ...
        across(name, \(x) str_replace_all(x, "group_activity_", "group-activity_")),
        across(value, \(x) if_else(str_detect(name, "case|soap"), x * 1.15, x))
    ) |>
    separate(name, c("group", "interviewer"), sep = "_") 
...
openxlsx::write.xlsx(
    data_export,
    paste0(f, "final/2026_interviews_phase_ii.xlsx")
)
